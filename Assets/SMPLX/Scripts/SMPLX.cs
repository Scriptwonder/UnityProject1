...
public class SMPLX : MonoBehaviour
{
    ...
    public int fixedFrame = 60;
    private int _fixedFrame = 1;

    string[] _testName = {"pelvis","left_hip","right_hip","spine1","left_knee","right_knee","spine2","left_ankle","right_ankle","spine3", "left_foot","right_foot"};
    ...
    Dictionary<string, Quaternion> _originalRotation = new Dictionary<string, Quaternion>();

    // Joint recalculation
    public static Dictionary<string, Matrix[]> JointMatrices = null;

    public float[,] npyArray;
    public float[,] translationArray;
    public List<GameObject> joints;

    public string csvFilePath = "Assets/SMPLX/my_file_newlong2.csv";
    public int totalFrame;
    public int jointNumber = 25;

    public float[,] prepareNpy() {
        // CSV Processing
        ...
        int numRows = rows.Length - 4;
        totalFrame = numRows/jointNumber;
        
        int numCols = rows[0].Split(',').Length;
        Debug.Log(numRows + " " + numCols + " " + jointNumber + " " + totalFrame);
        ...
        for (int i = 0; i < numRows; i++)
        {
            ...
            {
                float value;
                if (float.TryParse(fields[j], out value))
                {
                    dataArray[i, j] = value;
                    //Debug.Log(i + " " + j);
                }
                ...
                {
                    Debug.LogError("Error parsing CSV data at row " + i + ", column " + j);
                }
            }
        }
        translationArray = new float[3, totalFrame];
        for (int i = 0; i < 3; i++)
        {
            string[] fields = rows[i + numRows].Split(',');
            for (int j = 0; j < totalFrame; j++) {
                float value;
                if (float.TryParse(fields[j], out value))
                {
                    translationArray[i, j] = value;
                    //Debug.Log(i + " " + j);
                }
            }
        }
        Debug.Log("Data array Dimension: " + dataArray.GetLength(0) + " x " + dataArray.GetLength(1));
        return dataArray;
    }

    public void perserveOriginalTranslation() {
        for (int i = 0; i < 22; i++) {
            Transform joint = _transformFromName[_bodyJointNames[i]];
            Quaternion originalTranslation = joint.localRotation;
            _originalRotation.Add(_bodyJointNames[i], originalTranslation);
        }
    }

    public void testJointRotation(float[,] dataArray) {
        for (int i =0; i<22; i++) {
            Transform joint = _transformFromName[_bodyJointNames[i]];
            Quaternion quat = new Quaternion(dataArray[i*totalFrame, 0], dataArray[i*totalFrame, 1], dataArray[i*totalFrame, 2], dataArray[i*totalFrame, 3]);
            //joint.rotation = quat;
            SetLocalJointRotation(_bodyJointNames[i], quat);
            //Debug.Log("joint " + _bodyJointNames[i] + " local rotation: " + joint.localRotation.eulerAngles);
            //Vector3 euler = quat.eulerAngles;
            //Debug.Log("euler stats for joint " + joint.name + " : " + euler.x + " " + euler.y + " " + euler.z);
        }
        //Debug.Log("ok");
    }

    public void testJointRotationChange(int frame, float[,] dataArray) {
        for (int i =0; i<22; i++) {
            Transform joint = _transformFromName[_bodyJointNames[i+1]];
            Quaternion quat = new Quaternion(dataArray[i*totalFrame+frame, 0], dataArray[i*totalFrame+frame, 1], dataArray[i*totalFrame+frame, 2], dataArray[i*totalFrame+frame, 3]);
            //joint.rotation = quat;
            SetLocalJointRotation(_bodyJointNames[i], quat);
        }
        //Debug.Log("ok");
    ...
    public void testJointEuler(float[,] dataArray) {
        for (int i = 0; i < 22; i++) {
            Transform joint = _transformFromName[_bodyJointNames[i]];
            Vector3 euler = new Vector3(Mathf.Rad2Deg * dataArray[i * totalFrame, 0], -Mathf.Rad2Deg * dataArray[i * totalFrame, 1], -Mathf.Rad2Deg * dataArray[i * totalFrame, 2]);
            Quaternion quat = Quaternion.Euler(euler);
            joint.localRotation = quat * _originalRotation[_bodyJointNames[i]];
            //SetLocalJointEuler(_bodyJointNames[i + 1], euler);
            //Debug.Log("joint " + _bodyJointNames[i] + " local rotation: " + joint.localRotation.eulerAngles);
            Vector3 eu = quat.eulerAngles;
            Debug.Log("euler stats for joint " + joint.name + " : " + eu.x + " " + eu.y + " " + eu.z);
        }
    }
    public void testJointEulerChange(int frame, float[,] dataArray) {
        for (int i = 0; i < 22; i++) {
            Transform joint = _transformFromName[_bodyJointNames[i]];
            Vector3 euler = new Vector3(Mathf.Rad2Deg * dataArray[i * totalFrame + frame, 0], -Mathf.Rad2Deg * dataArray[i * totalFrame + frame, 1], -Mathf.Rad2Deg * dataArray[i * totalFrame + frame, 2]);
            if (i == 0) {
                //euler.y = Mathf.Lerp(0, 180f, frame/392f);
                //euler.x = 0;
                //euler.z = 0;
            }
            Quaternion quat = Quaternion.Euler(euler);
    ...
    public void rootTranslation(int frame, float[,] dataArray) {
        Vector3 newMove = new Vector3(translationArray[0, frame], translationArray[1, frame], -translationArray[2, frame]);
        this.gameObject.transform.localPosition = newMove;
    }


    public void testJointLocation(float[,] dataArray) {
        for (int i=0; i<24; i++){
                GameObject sphere = GameObject.CreatePrimitive(PrimitiveType.Sphere);
                //sphere.transform.position = new Vector3(dataArray[i*6, 0], dataArray[i*6+1, 0], dataArray[i*6+2, 0]);
                //sphere.transform.position = new Vector3(dataArray[i*6+3, 0], dataArray[i*6+4, 0], dataArray[i*6+5, 0]);
                sphere.transform.position = new Vector3(dataArray[i*6 + 150, 0], dataArray[i*6+151, 0], dataArray[i*6+152, 0]);
                sphere.transform.localScale = new Vector3(1f, 1f, 1f);
                joints.Add(sphere);
                //joint.position = new Vector3(dataArray[i*6, frame], dataArray[i*6+1, frame], dataArray[i*6+2, frame]);
                //joint.rotation = Quaternion.Euler(dataArray[i*6+3, frame], dataArray[i*6+4, frame], dataArray[i*6+5, frame]);
        }
    }

    public void testJointLocationChange(int frame, float[,] dataArray) {
        for (int i=0; i<24; i++){
                //joints[i].transform.position = new Vector3(dataArray[i*6, frame], dataArray[i*6+1, frame], dataArray[i*6+2, frame]);
                //joints[i].transform.position = new Vector3(dataArray[i*6+3, frame], dataArray[i*6+4, frame], dataArray[i*6+5, frame]);
                joints[i].transform.position = new Vector3(dataArray[i*6 + 150, frame], dataArray[i*6+151, frame], dataArray[i*6+152, frame]);
                //joint.position = new Vector3(dataArray[i*6, frame], dataArray[i*6+1, frame], dataArray[i*6+2, frame]);
                //joint.rotation = Quaternion.Euler(dataArray[i*6+3, frame], dataArray[i*6+4, frame], dataArray[i*6+5, frame]);
        }
    }

    public void runAnimation(int frame, float[,] dataArray) {
        for (int i=0; i<25; i++){
                Transform joint = _transformFromName[_bodyJointNames[i]];
                joint.position = new Vector3(dataArray[i*6, frame], dataArray[i*6+1, frame], dataArray[i*6+2, frame]);
                joint.rotation = Quaternion.Euler(dataArray[i*6+3, frame], dataArray[i*6+4, frame], dataArray[i*6+5, frame]);
    ...
    public void UpdateMesh() {
        //testJointEulerChange(currentFrame, npyArray);
        testJointRotationChange(currentFrame, npyArray);
        rootTranslation(currentFrame, npyArray);
    }
    ...
    public void Awake()
    {
        if (_transformFromName == null)
        {
            ...
            foreach (Transform t in transforms)
            {
                ...
                //Debug.Log("Added transform " + t.name);
            }
        }
    ...
    void FixedUpdate()
    {
        ...
        timer++;
        if (currentFrame < totalFrame && animated) {
            UpdateMesh();
            if (timer >= 1) {
                currentFrame += 1;
                timer = 0;
...
public class SMPLX_Editor : Editor {
    ...
    public override void OnInspectorGUI()
    {
        ...
        using (new EditorGUILayout.VerticalScope("Box")) 
        {
            ...
            //csv file input
            GUILayout.Label("CSV File Input", EditorStyles.boldLabel);
            TextAsset csvFile = null;

            // File path text input field
            string filePath = EditorGUILayout.TextField("File Path:", csvFile != null ? AssetDatabase.GetAssetPath(csvFile) : "");

            // CSV file object field
            ...
            if (GUILayout.Button("Read CSV"))
            {
                ...
                if (csvFile != null)
                {
                    ...
                    // Perform the desired operation with the CSV file
                    _target.csvFilePath = csvFilePath;
                    Debug.Log(csvFilePath);
                    _target.npyArray = _target.prepareNpy();
                }
                else if (!string.IsNullOrEmpty(filePath))
                {
                    // Perform the desired operation with the CSV file
                    //ReadCSV(filePath);
                    _target.csvFilePath = filePath;
                    _target.npyArray =  _target.prepareNpy();
                } else {
                    _target.npyArray =  _target.prepareNpy();
                }
            ...
            //Frame
            GUILayout.Label("Current Frame", EditorStyles.boldLabel);
            int currentFrame = EditorGUILayout.IntField("Frame", _target.currentFrame);
            if (currentFrame != _target.currentFrame)
            {
                _target.currentFrame = currentFrame;
                _target.UpdateMesh();
            }
            ...
            using (new EditorGUILayout.VerticalScope("Box"))
                {
                    float labelWidth = EditorGUIUtility.labelWidth;
                    EditorGUIUtility.labelWidth = 200;
                    bool animated = EditorGUILayout.Toggle("Animated", _target.animated);
                    if (animated != _target.animated)
                    {
                        _target.animated = animated;
                    }
                    EditorGUIUtility.labelWidth = labelWidth;
                }
